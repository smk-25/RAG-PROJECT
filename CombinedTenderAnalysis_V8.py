...
# --------------------------
st.set_page_config(
    page_title="Tender Analyzer Pro V6",
    page_icon="📋",
    layout="wide",
...
HYBRID_FUSION_RRF_WEIGHT = 1.0
HYBRID_FUSION_DENSE_WEIGHT = 0.5
HYBRID_FUSION_SPARSE_WEIGHT = 0.1
CROSS_ENCODER_WEIGHT = 0.7
ORIGINAL_SCORE_WEIGHT = 0.3
MULTI_QUERY_RRF_WEIGHT = 0.8
MULTI_QUERY_AVG_WEIGHT = 0.2
...
MULTI_QUERY_RETRIEVAL_MULTIPLIER = 4
CROSS_ENCODER_CANDIDATE_MULTIPLIER = 15
CLAUSE_PATTERNS = [
    r'\d+\.\d+(?:\.\d+)?',
    r'[A-Z][A-Za-z\s]+:',
    r'\([a-z]\)',
...
def compute_mean_support_score_shared(results: List[Dict[str, Any]]) -> float:
    ...
    scores = sorted([float(r.get("similarity_score", 0.0)) for r in results], reverse=True)
    # Refinement: Using mean of top 3 for better correlation with "best matches"
    return float(np.mean(scores[:3])) if scores else 0.0

def normalize_confidence_score(raw_score: float) -> float:
    """Simple linear clipping to preserve statistical correlation."""
    return float(np.clip(raw_score, 0.0, 1.0))

def compute_lexical_overlap(ans: str, res: List[Dict[str, Any]]) -> float:
    """Calculate word overlap between answer and context chunks using Counter to mirror F1 metrics."""
    if not ans or not res: return 0.0
    ans_tokens = tokenize_simple_shared(ans)
    # Filter out common stop-words and short noise
    ans_tokens = [t for t in ans_tokens if t not in _EVIDENCE_STOP_WORDS and len(t) > 2]
    if not ans_tokens: return 0.0
    
    ans_counts = collections.Counter(ans_tokens)
    max_overlap = 0.0
    for c in res:
        ctx_tokens = tokenize_simple_shared(c.get("content", ""))
        common = ans_counts & collections.Counter(ctx_tokens)
        overlap = sum(common.values()) / len(ans_tokens)
        if overlap > max_overlap: max_overlap = overlap
    return max_overlap

def get_answer_evidence_alignment(ans: str, res: List[Dict[str, Any]], em, cross_encoder=None) -> float:
    """Calculate semantic alignment between the answer and retrieved evidence chunks."""
    if not ans or not res: return 0.0
    ans_clean = strip_provenance(ans)
    if not ans_clean: return 0.0
    
    # 1. Vector similarity (Mean-of-Top-3 or Max if fewer)
    ans_emb = em.generate_embeddings([ans_clean])[0]
    chunk_embs = [c.get("emb") for c in res if c.get("emb") is not None]
    
    vector_sim = 0.0
    if chunk_embs:
        sims = cosine_similarity([ans_emb], chunk_embs)[0]
        # Refinement: Mean of top 3 for robustness
        vector_sim = float(np.mean(sorted(sims, reverse=True)[:3]))
        
    # 2. Cross-Encoder alignment (if available)
    cross_sim = 0.0
    if CROSS_ENCODER_AVAILABLE and cross_encoder is not None:
        try:
            # We take the top 3 chunks for cross-encoding to save time
            top_chunks = sorted(res, key=lambda x: x.get("similarity_score", 0.0), reverse=True)[:3]
            pairs = [[ans_clean, c["content"]] for c in top_chunks]
            scores = cross_encoder.predict(pairs)
            # Normalize scores using sigmoid
            norm_scores = [1.0 / (1.0 + np.exp(-float(s))) for s in scores]
            # Refinement: Mean of top 3 (already taking 3)
            cross_sim = float(np.mean(norm_scores))
        except:
            cross_sim = vector_sim # Fallback to vector sim
            
    # Combine (prefer cross-encoder if it exists)
    if cross_sim > 0:
        return float(0.3 * vector_sim + 0.7 * cross_sim)
    return float(vector_sim)

def compute_enhanced_confidence_rag(ans: str, retrieval_score: float, agreement_score: float, lexical_bonus: float, coverage_score: float) -> Dict[str, float]:
    """Calculate confidence using a Balanced-Hybrid model to maximize F1 correlation.
    
    Weights: Lexical (40%), Retrieval (25%), Semantic (25%), Coverage (10%).
    This balance ensures direct grounding (lexical) is prioritized while still
    valuing semantic relevance and source diversity.
    """
    # 1. Base Score calculation using the user-specified balanced formula
    raw_score = (0.55 * lexical_bonus + 
                 0.15 * retrieval_score + 
                 0.20 * agreement_score + 
                 0.10 * coverage_score)
    
    # 2. Safety Gates (Multiplicative Penalties)
    
    # Refusal Penalty: Non-answers should not get falsely high confidence scores
    ans_lower = (ans or "").lower()
    refusals = [
        "insufficient information", "not provided", "not explicitly state", 
        "no evidence found", "does not specify", "cannot be determined",
        "partial information available"
    ]
    if any(r in ans_lower for r in refusals):
        raw_score *= 0.15 # Massive penalty for failing to extract a definitive factual answer

    # Stricter Alignment Gates: Completely disjoint semantic vectors mean hallucination
    if agreement_score < 0.30:
        raw_score *= 0.2
    elif agreement_score < 0.50:
        raw_score *= 0.5
        
    # Minimum grounded Lexical requirement
    if lexical_bonus < 0.10:
        raw_score *= 0.3
        
    final_score = normalize_confidence_score(raw_score)
    
    return {
        "retrieval": retrieval_score,
        "alignment": agreement_score,
        "lexical": lexical_bonus,
...
def token_f1_shared(pred: str, gold: str) -> float:
    p = token_precision_shared(pred, gold)
    r = token_recall_shared(pred, gold)
...
class RAGRetrieverRAG:
    ...
    def retrieve(self, q, top_k=5):
        ...
        if self.cross is not None:
            # Optimize: Rerank more candidates (up to 50) to counteract sparse noise
            cands = self._rerank_with_cross_encoder(q, cands, min(50, top_k * CROSS_ENCODER_CANDIDATE_MULTIPLIER))
...
ANSWER:"""
    ...
    async def generate_response_async(self, q, ctx, prompt_instr=""):
        p = f"""{prompt_instr}
TASK: Answer the user's question using ONLY the provided context.
CONTEXT:
{ctx}
...
7. If the context contains table data (sections marked with [TABLE DATA - PAGE N TABLE N]: or formatted as Markdown tables with | separators), carefully read each column header and its corresponding row values to extract relevant information. Tables often contain key figures, schedules, specifications, or lists — do not skip them.
ANSWER:"""
        resp = await self.llm.ainvoke([HumanMessage(content=p)])
        return resp.content
...
Respond with ONLY the alternative questions, one per line. VARIATIONS:"""
    ...
    async def expand_query_async(self, q):
        try:
            prompt = f"""Generate {MAX_QUERY_VARIATIONS} alternative phrasings of: "{q}"
Respond with ONLY the alternative questions, one per line. VARIATIONS:"""
...
class QueryRouterRAG:
    """Intelligent router that classifies queries into appropriate pipelines.
    Uses a hybrid approach (heuristics + semantic statistics).
    """
    def __init__(self, em=None):
        self.em = em
        # Interpretation weights for the "learned baseline"
        self.weights = {
            "synthesis_keywords": 0.4,
            "query_length": 0.1,
            "max_similarity": -0.3, # High max sim suggests RAG
            "sim_variance": 0.2     # High variance suggests scattered info
        }

    def route_query(self, q: str, vector_store=None):
        """Returns ('Simple QA' or 'Contextual Understanding', explanation_dict)."""
        feats = {"synthesis_keywords": 0.0, "query_length": 0.0, "max_similarity": 0.0, "sim_variance": 0.0}
        
        # 1. Linguistic Heuristics
        q_lower = q.lower()
        synthesis_terms = ["summarize", "all", "every", "compare", "analysis", "overall", "comprehensive", "history", "trend"]
        feats["synthesis_keywords"] = sum(1 for t in synthesis_terms if t in q_lower) / len(synthesis_terms)
        feats["query_length"] = min(1.0, len(q.split()) / 20.0)
        
        # 2. Semantic Statistics (Initial Retrieval Pre-Scan)
        best_sim = 0.0
        std_sim = 0.0
        if self.em and vector_store:
            try:
                q_emb = self.em.generate_embeddings([q])[0]
                res = vector_store.collection.query(query_embeddings=[q_emb.tolist()], n_results=min(20, vector_store.collection.count()))
        ...
        feats["max_similarity"] = best_sim
        feats["sim_variance"] = std_sim
        
        # 3. Weighted Scoring
        score = (self.weights["synthesis_keywords"] * feats["synthesis_keywords"] + 
                 self.weights["query_length"] * feats["query_length"] + 
                 self.weights["max_similarity"] * feats["max_similarity"] + 
                 self.weights["sim_variance"] * feats["sim_variance"])
        
        # Threshold
        pipeline = "Simple QA"
        if score > 0.1 or feats["synthesis_keywords"] > 0.1:
            pipeline = "Contextual Understanding"
            
        return pipeline, {"score": score, "features": feats}

def assemble_context_rag(chunks, max_ctx=2048):
    """Filter and assemble context text, allowing partial truncation of the last chunk to maximize evidence."""
    parts = []
    used_chunks = []
    curr_chars = 0
    max_chars = max_ctx * CHARS_PER_TOKEN_ESTIMATE
    
    for c in chunks:
        p = f"Source: {c['metadata'].get('source_file')} (Page: {c['metadata'].get('page', 'N/A')}) [ID: {c['id'][:8]}]\n{c['content']}"
        rem_chars = max_chars - curr_chars
        
        if rem_chars <= 100: # Threshold for usable text
            break
            
        if len(p) > rem_chars:
            # Partial truncation: preserve the start of the chunk
            p = p[:rem_chars-20] + "... [TRUNCATED]"
            parts.append(p)
            used_chunks.append(c)
            break # Context is now full
        else:
            parts.append(p)
            used_chunks.append(c)
            curr_chars += len(p) + 2 
...
_RAG_CONCURRENCY_MGR = ConcurrencyManager(2) # Lower concurrency for Groq to avoid 429s

async def run_bulk_evaluation_rag(eval_df, vs, em, llm, cr, sparse_index, top_k, max_tk, use_qe, use_spa):
    """Asynchronous execution loop for bulk evaluation of Simple QA mode."""
    results = []
    sem = _RAG_CONCURRENCY_MGR.get()
    ret = RAGRetrieverRAG(vs, em, cr, sparse=sparse_index, use_s=use_spa)
    
    async def process_query(row):
        q_id = str(row["id"]).strip()
        # Simplified sanitization to match single-query path exactly (parity fix)
        q_raw = str(row["question"])
        q_text = q_raw.replace('\ufeff', '').strip()
        ...
        try:
            async with (sem if sem else asyncio.Lock()):
                # 1. Retrieval
                is_sparse_ready = ret.use_s and (ret.sparse and ret.sparse.is_ready())
                if use_qe:
                    queries = await llm.expand_query_async(q_text)
                    res = await loop.run_in_executor(None, multi_query_retrieval, ret, queries, top_k)
                else:
                    res = await loop.run_in_executor(None, ret.retrieve, q_text, top_k)
                
                t_ret = time.time() - t_start
                # Detailed Diagnostic Logging
                print(f"[BULK EVAL] ID: {q_id} | Q: '{q_text}'")
                print(f"            - Sparse Ready: {is_sparse_ready} | QE: {use_qe}")
                print(f"            - Retrieved: {len(res)} chunks in {t_ret:.2f}s")
                if res:
                    print(f"            - Top Chunk ID: {res[0]['id'][:8]} | Score: {res[0].get('similarity_score', 0.0):.4f}")
                
                if not res:
                    return {
                        "id": q_id, "question": q_text, "generated_answer": "No evidence found (0 chunks retrieved).", "gold_answer": gold,
                        "f1": 0, "overall_confidence": 0
                    }
                ...
                for attempt in range(3):
                    try:
                        ans = await llm.generate_response_async(q_text, ctx)
                        if ans: break
                    except Exception as gen_e:
                        if "429" in str(gen_e): await asyncio.sleep(5)
                ...
                # 3. Metrics & Confidence
                # These are CPU bound but fast enough to run sequentially here
                prec = token_precision_shared(ans_clean, gold)
                recall = token_recall_shared(ans_clean, gold)
                f1 = token_f1_shared(ans_clean, gold)
                rouge_l = rouge_l_shared(ans_clean, gold)
                
                retrieval_conf = compute_mean_support_score_shared(res)
                semantic_score = get_answer_evidence_alignment(ans_clean, res, em, cross_encoder=cr)
                lexical_bonus = compute_lexical_overlap(ans_clean, res)
                
                # Refinement: True Coverage based on chunks actually cited/used by the LLM
                s_map_cov = map_sentences_to_sources_rag(ans_clean, used, em)
                cited_chunk_ids = set([m.get("chunk_id", "") for m in s_map_cov])
                raw_cov = len(cited_chunk_ids) / max(1, len(res))
                coverage_score = max(0.3, raw_cov)
                
                agreement_score = 0.7 * semantic_score + 0.3 * lexical_bonus
                
                conf_results = compute_enhanced_confidence_rag(ans_clean, retrieval_conf, agreement_score, lexical_bonus, coverage_score)
                
                return {
                    "id": q_id, "question": q_text, "generated_answer": ans_clean, "gold_answer": gold,
                    "retrieval_time": t_ret, "generation_time": t_gen, "total_time_secs": t_total,
                    "precision": prec, "recall": recall, "f1": f1, "rouge_l": rouge_l,
                    "retrieval_conf": retrieval_conf, "semantic_conf": agreement_score,
                    "lexical_conf": lexical_bonus, "coverage_conf": coverage_score,
                    "overall_confidence": conf_results["overall"]
                }
        except Exception as e:
            return {"id": q_id, "question": q_text, "generated_answer": f"Error: {e}", "gold_answer": gold, "f1": 0}

    # Use asyncio.gather to process queries in parallel
    tasks = [process_query(row) for _, row in eval_df.iterrows()]
...
async def call_gemini_json_sum_async(client, sys, user, model, rpm):
    """
    Async wrapper for Vertex AI GenerativeModel.generate_content.
    `client` is a GenerativeModel instance (initialised before calling).
    The system instruction is prepended to the user prompt because Vertex AI's
    generate_content accepts a flat list of content parts, not a config object.
    Uses ConcurrencyManager + TokenBucket rate limiting, 20-retry exponential back-off.
    """
    limiter = get_sum_limiter(rpm)
    backoff = 5.0
    loop = asyncio.get_event_loop()
    # Combine system + user prompts into a single content string for Vertex AI
    full_prompt = f"{sys}\n\n{user}"
    for attempt in range(20):
        try:
            await limiter.wait()
            sem = _SUM_CONCURRENCY_MGR.get()
            # Run the synchronous Vertex AI SDK call in a thread executor so we
            # remain non-blocking inside the asyncio event loop.
            def _vertex_call():
            ...
            if sem:
                async with sem:
                    resp = await loop.run_in_executor(None, _vertex_call)
            else:
                resp = await loop.run_in_executor(None, _vertex_call)
            txt = clean_json_string(resp.text or "{}")
            # Forcefully remove invalid unicode escapes that Gemini sometimes generates
            txt = re.sub(r'\\u[0-9a-fA-F]{0,3}[^0-9a-fA-F]', '', txt)
            try:
...
def compute_confidence_score_sum(mapped, reduced, q, em=None):
    conf = {"snippet_coverage": 0.0, "result_coherence": 0.0, "information_density": 0.0, "citation_confidence": 0.0, "consistency_score": 0.0, "uncertainty_penalty": 0.0, "overall_confidence": 0.0}
    if not mapped: return conf
    # 1. Snippet Coverage: 3+ snippets for full score (25% weight)
    conf["snippet_coverage"] = min(1.0, len(mapped) / 3.0)
    
    # 2. Result Coherence: 1+ significant field for full score (20% weight)
    coh = 0.0
    ...
    # 3. Information Density: Better sweet spot (15% weight)
    ...
    # 4. Citation Confidence: 1 citation per 3 snippets (15% weight)
    cites = []
    if isinstance(reduced, dict):
    ...
    # 5. Advanced Semantic Metrics (Consistency & Uncertainty)
    if em:
        try:
            ev_texts = []
            
            def _find_ev_recursive(obj):
                if isinstance(obj, dict):
                    if obj.get("evidence"):
                        ev_texts.append(str(obj["evidence"]))
                    for v in obj.values():
                        _find_ev_recursive(v)
                elif isinstance(obj, list):
                    for v in obj:
                        _find_ev_recursive(v)

            # Collect from mapped items
            for m in mapped:
                _find_ev_recursive(m)
            # Collect from reduced result
            _find_ev_recursive(reduced)
            
            # De-duplicate and filter
            evidences = [ev for ev in set(ev_texts) if ev and len(ev.strip()) > 5]
            
            if len(evidences) >= 1:
                e_embs = em.generate_embeddings(evidences)
                q_emb = em.generate_embeddings([q])[0]
                
                # Consistency (15% weight)
                if len(e_embs) > 1:
                    sim_mat = cosine_similarity(e_embs)
                    n = len(e_embs)
                    pairwise_sim = (np.sum(sim_mat) - n) / (n * (n - 1))
                    conf["consistency_score"] = float(np.clip(pairwise_sim, 0, 1))
                else:
                    conf["consistency_score"] = 1.0 
                
                # Uncertainty Penalty (10% weight)
                q_sims = cosine_similarity([q_emb], e_embs)[0]
                std_dev = np.std(q_sims)
                conf["uncertainty_penalty"] = float(np.clip(1.0 - (std_dev / 0.15), 0, 1))
            else:
                # If no evidence text found, at least give a baseline if mapped is not empty
                if mapped:
                    conf["consistency_score"] = 0.5
                    conf["uncertainty_penalty"] = 0.5
        except Exception:
            pass

    # Weights: Coverage (25%), Coherence (20%), Density (15%), Citation (15%), Consistency (15%), Uncertainty (10%)
    conf["overall_confidence"] = (0.25 * conf["snippet_coverage"] + 0.20 * conf["result_coherence"] + 
                                  0.15 * conf["information_density"] + 0.15 * conf["citation_confidence"] + 
                                  0.15 * conf["consistency_score"] + 0.10 * conf["uncertainty_penalty"])
    return conf

async def run_bulk_evaluation_deep(eval_df, chunks, client, s_obj, s_batch, s_model, em, s_rpm):
    """Asynchronous execution loop for bulk evaluation of Deep Analysis mode."""
    results = []
    _em_conf = em
    ms, mi, rs, ri = get_sum_prompts(s_obj)
    
    # Pre-batch context chunks to avoid repeated overhead
    ...
    for i, (_, row) in enumerate(eval_df.iterrows()):
        ...
        print(f"\n[DEEP EVAL] ID {q_id} | Query: {q_text[:100]}...")
        
        t0 = time.time()
        try:
            # 1. Parallel Map Phase
            mapped_batches = await asyncio.gather(
                *[call_gemini_json_sum_async(client, ms, f"{mi}\nQ:{q_text}\nContext Data:\n{p}", s_model, s_rpm) for p in chunk_payloads],
                return_exceptions=True
            )
            ...
            for mb in mapped_batches:
                if not isinstance(mb, Exception):
                    mapped.extend(_unwrap_batch_result(mb, s_obj))
            
            # 2. Hierarchical Reduce Phase
            red = await _hierarchical_reduce(client, rs, ri, q_text, mapped, s_model, s_rpm, s_obj)
            red = _apply_result_fallback(mapped, red, s_obj)
            _recover_evidence_in_result(mapped, red, s_obj, chunks)
            
            # 3. Confidence Calculation
            _conf = compute_confidence_score_sum(mapped, red, q_text, em=_em_conf)
            _m_count = len(mapped)
            
            # 4. Extract answer text for metrics comparison
            pred_text = ""
            if s_obj in ("General Summary", "Overall Summary & Voice"):
                pred_text = str(red.get("summary", ""))
            else:
                # Use narrative summary if available, otherwise fallback to overall assessment or serialized JSON
                pred_text = str(red.get("summary") or red.get("overall_assessment") or "")
                if not pred_text:
                    if s_obj == "Compliance Matrix": items = red.get("matrix", [])
                    elif s_obj == "Risk Assessment": items = red.get("risks", [])
                    elif s_obj == "Entity Dashboard": items = list((red.get("dashboard") or {}).values())
                    elif s_obj == "Ambiguity Scrutiny": items = red.get("ambiguities", [])
                    else: items = []
                    
                    if items: pred_text = json.dumps(items)
                    else: pred_text = json.dumps(red)

            # 5. Metric Scoring
            f1 = token_f1_shared(pred_text, gold)
            prec = token_precision_shared(pred_text, gold)
            rec = token_recall_shared(pred_text, gold)
            rouge = rouge_l_shared(pred_text, gold)
            
            results.append({
                "id": q_id, "question": q_text, "gold": gold, "answer": pred_text,
                "f1": f1, "precision": prec, "recall": rec, "rouge_l": rouge,
                "overall_conf": _conf.get("overall_confidence", 0.0),
                "snippet_cov": _conf.get("snippet_coverage", 0.0),
                "coherence": _conf.get("result_coherence", 0.0),
                "density": _conf.get("information_density", 0.0),
                "citation_conf": _conf.get("citation_confidence", 0.0),
                "consistency": _conf.get("consistency_score", 0.0),
                "uncertainty": _conf.get("uncertainty_penalty", 0.0),
                "time": time.time() - t0
            })
            print(f"       Completed in {results[-1]['time']:.1f}s | F1: {f1:.2f} | Conf: {results[-1]['overall_conf']:.2f}")
        except Exception as e:
            print(f"       ERROR on ID {q_id}: {e}")
            results.append({"id": q_id, "question": q_text, "gold": gold, "answer": f"ERROR: {e}", "f1": 0, "precision": 0, "recall": 0, "rouge_l": 0, "overall_conf": 0, "time": time.time() - t0})
...
with st.sidebar:
    ...
    routing_policy = st.radio("Routing Policy", ["Manual", "Automatic (Hybrid AI)"], help="Auto mode uses AI to choose between Simple lookup and Deep analysis.")
    
    if routing_policy == "Manual":
        choice = st.radio("Navigation", ["⚡ Simple QA (RAG)", "🧠 Context Understanding"], label_visibility="collapsed")
    else:
        choice = "Auto"
        st.info("🤖 Routing query automatically based on intent and document distribution.")
    
    st.markdown("---")
    st.subheader("⚙️ Global Settings")
    
    # Common settings for both modes
    with st.expander("🛠️ Common Model Settings", expanded=True):
        ...
        u_spa = st.checkbox("Use Sparse BM25", True, key="u_spa_global")

    # Show Groq settings if Simple QA is manually selected OR if in Auto mode
    if choice == "⚡ Simple QA (RAG)" or choice == "Auto":
        with st.expander("⚡ RAG Settings", expanded=(choice == "⚡ Simple QA (RAG)")):
    ...
    # Show Vertex settings if Context Understanding is manually selected OR if in Auto mode
    if choice == "🧠 Context Understanding" or choice == "Auto":
        with st.expander("🧠 Deep Analysis Settings", expanded=(choice == "🧠 Context Understanding")):
            vertex_project = st.text_input("Vertex AI Project ID", value="project-58068847-72c4-4cbf-bb5")
            vertex_location = st.text_input("Vertex AI Location", value="us-central1")
            ...
            with st.expander("Advanced Summary Tuning"):
                ...
                s_rpm = st.number_input("Target RPM", 1, 300, 30)
                s_batch = st.number_input("Batch size", 5, 100, 20)
...
if choice == "⚡ Simple QA (RAG)" or choice == "Auto":
    if choice == "Auto":
        st.markdown('<div class="app-header"><h1>🤖 Intelligent Hybrid Analysis</h1><p>Automatic routing between lightning-fast lookup and deep logical extraction.</p></div>', unsafe_allow_html=True)
    else:
    ...
    async def run_single_deep_query(q, chunks, client, s_obj, s_batch, s_model, _em_conf, s_rpm):
        """Core Map-Reduce logic for a single query."""
        ms, mi, rs, ri = get_sum_prompts(s_obj)
        batches = [chunks[i:i+s_batch] for i in range(0, len(chunks), s_batch)]
        chunk_payloads = [
            "\n\n".join(f"---\nID:{c['id']} P:{_chunk_page_label(c)}\nText: {c['text']}" for c in b)
        ...
        # CONFIDENCE
        _conf = compute_confidence_score_sum(mapped, red, q, em=_em_conf)
    ...
    def render_deep_analysis_per_query(r, s_obj, chunks, is_auto=False, routing_reason=None):
        """Unified renderer for a single Deep Analysis result."""
        st.markdown('<div class="app-header"><h3>🧠 Contextual Understanding Result</h3></div>', unsafe_allow_html=True)
        if is_auto and routing_reason:
            st.caption(f"✨ {routing_reason}")
            
        conf = r.get('confidence', r.get('conf', {}))
        render_metric_cards(r.get('mapped_count', 0), r.get('time', r.get('total_t', 0.0)), conf.get('overall_confidence', 0.0))

        with st.expander("🔍 Confidence Score Breakdown", expanded=False):
            sc_col1, sc_col2, sc_col3, sc_col4, sc_col5, sc_col6 = st.columns(6)
            score_details = [
                ("📄 Snippet Coverage",    conf.get('snippet_coverage', 0.0),    "Weight: 25%", "Number of context chunks mapped to the query"),
                ("🧩 Result Coherence",    conf.get('result_coherence', 0.0),    "Weight: 20%", "Completeness of structured AI output fields"),
                ("📊 Information Density", conf.get('information_density', 0.0), "Weight: 15%", "Richness of response relative to query length"),
                ("📌 Citation Confidence", conf.get('citation_confidence', 0.0), "Weight: 15%", "Unique page citations grounding the result"),
                ("🔗 Semantic Consistency", conf.get('consistency_score', 0.0),    "Weight: 15%", "Logical agreement between extracted evidence points"),
                ("⚖️ Uncertainty Penalty",  conf.get('uncertainty_penalty', 0.0), "Weight: 10%", "Statistical penalty for high variance in evidence relevance"),
            ]
            for col, (label, score, weight, desc) in zip([sc_col1, sc_col2, sc_col3, sc_col4, sc_col5, sc_col6], score_details):
        ...
        visual_tab = atabs[0]
        if s_obj == "Overall Summary & Voice":
            audio_tab = atabs[1]; context_tab = atabs[2]; export_tab = atabs[3]
        else:
            context_tab = atabs[1]; export_tab = atabs[2]
            
        with visual_tab:
            # Reusing the dashboard/logic from the dedicated mode
            # For simplicity in this function, we'll call the logic directly or refer to the session state logic
            # However, since we want Word/Excel to work, we'll focus on them in the export tab.
            if s_obj == "Entity Dashboard" and "dashboard" in r["result"]:
                ...
                if cat_counts:
                    ...
                    for i, (cat, cnt) in enumerate(cat_counts.items()):
                        with cc_cols[i % len(cc_cols)]: st.metric(cat, cnt)
                    st.markdown("---")
                for cat, items in dash.items():
                    if items and isinstance(items, list):
                        display_ents = [e for e in items if isinstance(e, dict)][:MAX_DISPLAY_ITEMS]
                        if display_ents: st.markdown(f"**{cat}** ({len(items)})")
                        for idx, ent in enumerate(display_ents):
                            with st.expander(f"{idx+1}. {ent.get('entity', 'Entity')} | Pages: {ent.get('pages', [])}"):
                                st.write(f"**Context:** {ent.get('context', 'N/A')}")
                                if ent.get('evidence'): st.markdown(f"**Evidence:** *\"{ent['evidence']}\"*")
            elif s_obj == "Risk Assessment" and "risks" in r["result"]:
                risks = r["result"]["risks"]
                for idx, risk in enumerate(risks[:MAX_DISPLAY_ITEMS]):
                    with st.expander(f"{idx+1}. {risk.get('risk_type', 'Risk')} — {risk.get('risk_level', 'Unknown')} severity"):
                        st.write(f"**Clause:** {risk.get('clause', 'N/A')}")
                        st.write(f"**Reason:** {risk.get('reason', 'N/A')}")
                        if risk.get('evidence'): st.markdown(f"**Evidence:** *\"{risk['evidence']}\"*")
            elif s_obj == "Ambiguity Scrutiny" and "ambiguities" in r["result"]:
                ambs = r["result"]["ambiguities"]
                for idx, a in enumerate(ambs[:MAX_DISPLAY_ITEMS]):
                    with st.expander(f"{idx+1}. {a.get('ambiguity_type', 'Ambiguity')} — {a.get('severity', 'Medium')} | Ref: {a.get('ambiguous_text', '')[:60]}"):
                        st.write(f"**Issue:** {a.get('issue', 'N/A')}")
                        if a.get('evidence'): st.markdown(f"**Evidence:** *\"{a['evidence']}\"*")
                ...
                for idx, item in enumerate(matrix_items[:MAX_DISPLAY_ITEMS]):
                    mandatory_badge = "🔴 Mandatory" if _resolve_mandatory_val(item.get("mandatory")) else "🟡 Optional"
                    with st.expander(f"{idx+1}. {item.get('item', 'Requirement')} — {mandatory_badge}"):
                        st.write(f"**Detail:** {item.get('detail', 'N/A')}")
                        if item.get('evidence'): st.markdown(f"**Evidence:** *\"{item['evidence']}\"*")
            else:
                ...
                if isinstance(o, dict):
                    for k, v in o.items():
                        if k.lower() in ["page", "pages", "citations"]:
                            if isinstance(v, list): [pgs.append(int(x)) for x in v if str(x).isdigit()]
                            elif str(v).isdigit(): pgs.append(int(v))
                        else: _f(v)
                elif isinstance(o, list): [_f(x) for x in o]
            _f(r['result'])
            if pgs: render_page_level_citations_sum(chunks, sorted(set(pgs)))
            else: st.info("No page citations found.")

        with export_tab:
            df = convert_result_to_dataframe(r["result"], s_obj)
            ex = export_to_excel(df, r['query'], s_obj) if df is not None else None
            wd = export_to_word(df, r['query'], s_obj, result=r['result'])
            if ex or wd:
                c1, c2 = st.columns(2)
                if ex: c1.download_button("📥 Excel", ex, f"{sanitize_filename(r['query'])}.xlsx", use_container_width=True)
                if wd: c2.download_button("📥 Word", wd, f"{sanitize_filename(r['query'])}.docx", use_container_width=True)
            st.subheader("Raw AI Output")
            st.json(r["result"])

    if "rvs_v2" in st.session_state:
        # Move display options to main area for better visibility and to ensure parameters are defined
        ...
        # Bulk Evaluation (CSV)
        with st.expander("📊 Bulk Evaluation (CSV)", expanded=False):
            eval_file = st.file_uploader("Upload Evaluation CSV", type="csv", key="eval_csv_uploader")
            if eval_file:
                if "last_eval_name" not in st.session_state or st.session_state["last_eval_name"] != eval_file.name:
                    if "eval_results" in st.session_state: del st.session_state["eval_results"]
                    st.session_state["last_eval_name"] = eval_file.name
                
                try:
                    df_eval = pd.read_csv(eval_file)
                    if all(col in df_eval.columns for col in ["id", "question", "gold"]):
                        st.success(f"CSV loaded: {len(df_eval)} questions.")
                        # Ensure IDs are stripped for consistent dropdown and range matching
                        ids = sorted(df_eval["id"].astype(str).str.strip().unique())
                        if ids:
                            col1, col2 = st.columns(2)
                            start_id = col1.selectbox("Start ID", ids, index=0)
                            end_id = col2.selectbox("End ID", ids, index=len(ids)-1)
                            
                            if st.button("🚀 Run Bulk Evaluation", use_container_width=True):
                                if not ga_key: st.error("Groq API Key Mandatory"); st.stop()
                                
                                try:
                                    # Use a more robust matching strategy for IDs (strip and stringify)
                                    df_eval["_id_str"] = df_eval["id"].astype(str).str.strip()
                                    target_ids = [str(start_id).strip(), str(end_id).strip()]
                                    s_idx = ids.index(start_id)
                                    e_idx = ids.index(end_id)
                                    if s_idx > e_idx:
                                        st.error("Start ID must be before or equal to End ID."); st.stop()
                                    range_ids = ids[s_idx : e_idx + 1]
                                    selected_eval = df_eval[df_eval["_id_str"].isin(range_ids)]
                                except Exception as e:
                                    st.error(f"Selection error: {e}"); st.stop()
                                
                                with st.status("🚀 Running Parallel Bulk Evaluation...", expanded=True) as status:
                                    em = st.session_state["sem_v2"]
                                    vs = st.session_state["rvs_v2"]
                                    llm = GroqLLMRAG(key=ga_key)
                                    cr = load_cross_encoder_cached(crs_m) if u_crs and CROSS_ENCODER_AVAILABLE else None
                                    
                                    # Use the new async bulk evaluation logic
                                    results_df = asyncio.run(run_bulk_evaluation_rag(
                                        selected_eval, vs, em, llm, cr, st.session_state.get("rbm_v2"), top_k, max_tk, use_qe, u_spa
                                    ))
                    ...
                    if "eval_results" in st.session_state:
                        df_res = st.session_state["eval_results"].copy()
                        # Ensure numeric conversion for aggregation, handling potential string formatted leftovers
                        metric_cols = ["f1", "precision", "recall", "rouge_l", "total_time_secs"]
                        for col in metric_cols:
                            if df_res[col].dtype == object:
                                df_res[col] = df_res[col].astype(str).str.replace('%', '').str.replace('s', '')
                                df_res[col] = pd.to_numeric(df_res[col], errors='coerce')
                            # If it was a percentage string (e.g. 92.31), we might need to divide by 100 if we want the true float
                            # But since we store raw floats now, this is just a safety measure for old data.
                        
                        st.markdown("### 📈 Aggregated Metrics")
                        avg_col1, avg_col2, avg_col3, avg_col4, avg_col5 = st.columns(5)
                        
                        f1_avg = df_res["f1"].mean()
                        p_avg = df_res["precision"].mean()
                        r_avg = df_res["recall"].mean()
                        rl_avg = df_res["rouge_l"].mean()
                        t_avg = df_res["total_time_secs"].mean()
                        
                        avg_col1.metric("Avg F1", f"{f1_avg:.2%}")
                        avg_col2.metric("Avg Precision", f"{p_avg:.2%}")
                        avg_col3.metric("Avg Recall", f"{r_avg:.2%}")
                        avg_col4.metric("Avg Rouge-L", f"{rl_avg:.2%}")
                        avg_col5.metric("Avg Total Time", f"{t_avg:.2f}s")

                        st.markdown("---")
                        st.markdown("### 📊 Detailed Evaluation Results")
                        # Format dataframe for display
                        display_df = df_res.copy()
                        for col in ["precision", "recall", "f1", "rouge_l", "retrieval_conf", "semantic_conf", "lexical_conf", "coverage_conf", "overall_confidence"]:
                            display_df[col] = display_df[col].map(lambda x: f"{x:.2%}")
                        for col in ["retrieval_time", "generation_time", "total_time_secs"]:
                            display_df[col] = display_df[col].map(lambda x: f"{x:.2f}s")
                        
                        st.dataframe(display_df, use_container_width=True)
                        csv_data = df_res.to_csv(index=False).encode('utf-8')
                        st.download_button("📥 Download Full Results CSV", csv_data, "evaluation_results.csv", "text/csv")
                except Exception as e:
                    st.error(f"Error loading CSV: {e}")
            else:
                st.info("Upload a CSV file with 'id', 'question', 'gold' columns to run bulk evaluation.")

        # Question input area
        if choice == "Auto":
            q_input = st.text_area("Analysis Queries (one per line):", placeholder="e.g., What are the penalties for delay?\nSummarize the technical delivery risks...")
            btn_label = "🚀 Analyze with AI Router"
            f_rag = files
        else:
            q_input = st.text_input("Enter your question about the uploaded tenders:")
            btn_label = "🔍 Answer"
            f_rag = files
        
        if st.button(btn_label, use_container_width=True):
            if not ga_key: st.error("Groq API Key Mandatory"); st.stop()
            q_list = [l.strip() for l in q_input.splitlines() if l.strip()]
            
            # Deep Analysis context will only be extracted if/when a query actually routes to it
            chunks_sum = None
            ...
            for q in q_list:
                ...
                if choice == "Auto":
                    router = QueryRouterRAG(em=em)
                    pipeline, info = router.route_query(q, vs)
                    routing_reason = f"AI routed to **{pipeline}** (Score: {info['score']:.2f})"
                    st.write(f"🤖 {routing_reason}")

                if pipeline == "Simple QA":
                    with st.status(f"RAG Analysis: {q}...", expanded=True) as status:
                        t_start = time.time()
                        cr = load_cross_encoder_cached(crs_m) if u_crs and CROSS_ENCODER_AVAILABLE else None
                        llm = GroqLLMRAG(key=ga_key)
                        ret = RAGRetrieverRAG(vs, em, cr, sparse=st.session_state.get("rbm_v2"), use_s=u_spa)
                        if use_qe:
                        ...
                        t_total = time.time() - t_start
                        status.update(label=f"RAG Complete", state="complete", expanded=False)

                    st.markdown('<div class="result-box">', unsafe_allow_html=True)
                    if choice == "Auto": st.caption(f"✨ {routing_reason}")
                    st.write(strip_provenance(ans))
                    st.markdown('</div>', unsafe_allow_html=True)

                    retrieval_score = compute_mean_support_score_shared(res)
                    ans_clean = strip_provenance(ans)
                    semantic_score = get_answer_evidence_alignment(ans_clean, res, em, cross_encoder=cr)
                    lexical_bonus = compute_lexical_overlap(ans_clean, res)
                    ...
                    # Enhanced Weighted Formula and Normalization
                    conf_results = compute_enhanced_confidence_rag(ans_clean, retrieval_score, agreement_score, lexical_bonus, coverage_score)
                    final_confidence = conf_results["overall"]
                    
                    render_metric_cards(len(res), t_total, final_confidence)
                    
                    tab1, tab2, tab3 = st.tabs(["📚 Evidence Chunks", "📋 Precise Citations", "✅ Validation"])
                    with tab1:
                        for i, c in enumerate(res):
                            with st.expander(f"Chunk {i+1} | Score: {c.get('similarity_score',0.0):.3f} | Page: {c['metadata'].get('page')}"): st.text(c['content'])
                    with tab2:
                        if show_cl:
                            c_map = map_answer_to_clauses_rag(strip_provenance(ans), used, em)
                            for m in c_map: st.caption(f"**{m['answer_sentence']}** → *{m['source_file']} P{m['page']}*"); st.write(f"> {m['source_clause']}")
                        if show_sc:
                            s_map = map_sentences_to_sources_rag(strip_provenance(ans), used, em)
                            for m in s_map: st.caption(f"**{m['answer_sentence']}** → *{m['source_file']} P{m['page']}*"); st.write(f"> {m['source_sentence']}")
                    with tab3:
                        st.markdown("#### 📊 Balanced Confidence Score Breakdown")
                        st.caption("Optimized for maximum F1 correlation (Balanced: Lexical/Semantic/Retrieval)")
                        c1, c2, c3, r_col = st.columns(4)
                        c3.metric("Lexical Support (55%)", f"{lexical_bonus:.1%}")
                        c1.metric("Retrieval Baseline (15%)", f"{retrieval_score:.1%}")
                        c2.metric("Semantic Alignment (20%)", f"{agreement_score:.1%}")
                        r_col.metric("Source Coverage (10%)", f"{coverage_score:.1%}")
                        st.info(f"The final score of **{final_confidence:.1%}** uses a balanced weighting of direct word overlap (Lexical), semantic similarity (Alignment), and retrieval reliability to ensure high correlation with factual accuracy. Multiplicative safety gates are applied to penalize potential hallucinations if any signal falls too low.")
                        if use_av:
                            is_v, msg = llm.validate_answer(q, ans, ctx)
                            if is_v: st.success(msg)
                            else: st.warning(msg)
                
                else: # Contextual Understanding
                    # Lazy extraction: Only extract the full text if we actually need deep analysis
                    if chunks_sum is None:
                        with st.spinner("Extracting entire document for Deep Analysis..."):
                            all_p, all_f = [], []
                            for f in f_rag:
                                with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as t:
                                    t.write(f.getbuffer())
                                ...
                                os.unlink(_tmp_path)
                            chunks_sum = semantic_chunk_pages_sum(all_p, all_f, max_tok=int(s_max_tk), overlap=int(s_over))

                    with st.status(f"Deep Analysis: {q}...", expanded=True) as status:
                        vertexai.init(project=vertex_project, location=vertex_location)
                        client = GenerativeModel(s_model)
                        q_res = asyncio.run(run_single_deep_query(q, chunks_sum, client, s_obj, s_batch, s_model, em, s_rpm))
                        status.update(label="Deep Analysis Complete", state="complete", expanded=False)
                    render_deep_analysis_per_query(q_res, s_obj, chunks_sum, is_auto=(choice=="Auto"), routing_reason=routing_reason)
...
# --------------------------
    ...
    with st.expander("📊 Bulk Evaluation (CSV)", expanded=False):
        st.info("Upload a CSV with 'id', 'question', and 'gold' columns to measure Deep Analysis performance.")
        e_file_sum = st.file_uploader("Upload Evaluation CSV", type="csv", key="e_file_sum")
        if e_file_sum:
            try:
                e_df_sum = pd.read_csv(e_file_sum)
                if not all(k in e_df_sum.columns for k in ["id", "question", "gold"]):
                    st.error("CSV must contain 'id', 'question', and 'gold' columns.")
                else:
                    # Ensure IDs are stripped for consistent dropdown and range matching
                    ids = sorted(e_df_sum["id"].astype(str).str.strip().unique())
                    if ids:
                        col1, col2 = st.columns(2)
                        start_id = col1.selectbox("Start ID", ids, index=0, key="sum_start_id")
                        end_id = col2.selectbox("End ID", ids, index=len(ids)-1, key="sum_end_id")
                        
                        if st.button("🏁 Run Deep Bulk Evaluation", use_container_width=True):
                            if not f_sum or not vertex_project:
                                st.error("Please upload PDFs and configure Vertex AI first.")
                            else:
                                try:
                                    # Use a more robust matching strategy for IDs
                                    e_df_sum["_id_str"] = e_df_sum["id"].astype(str).str.strip()
                                    target_ids = [str(start_id).strip(), str(end_id).strip()]
                                    s_idx = ids.index(start_id)
                                    e_idx = ids.index(end_id)
                                    if s_idx > e_idx:
                                        st.error("Start ID must be before or equal to End ID."); st.stop()
                                    range_ids = ids[s_idx : e_idx + 1]
                                    selected_eval = e_df_sum[e_df_sum["_id_str"].isin(range_ids)]
                                except Exception as e:
                                    st.error(f"Selection error: {e}"); st.stop()
                                
                                with st.status("Running Deep Bulk Evaluation...", expanded=True) as status:
                                    # Re-use indexing logic if needed or ensure chunks are ready
                                    temp_files = []
                                    try:
                                        ...
                                        for f in f_sum:
                                            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as t:
                                                t.write(f.getbuffer()); t.flush(); temp_files.append(t.name)
                                                pages, flags = extract_pages_sum(t.name)
                                                all_pages.extend(pages)
                                        
                                        chunks = semantic_chunk_pages_sum(all_pages, [False]*len(all_pages), max_tok=int(s_max_tk), overlap=int(s_over))
                                        st.write(f"Generated {len(chunks)} chunks.")
                                        
                                        client = GenerativeModel(s_model)
                                        em = _get_em(emb_m) # Reuse RAG embedding model for confidence consistency
                                        
                                        # Run the async evaluation loop
                                        eval_loop = asyncio.new_event_loop()
                                        asyncio.set_event_loop(eval_loop)
                                        res_df = eval_loop.run_until_complete(run_bulk_evaluation_deep(selected_eval, chunks, client, s_obj, int(s_batch), s_model, em, s_rpm))
                                        eval_loop.close()
                                        
                                        st.session_state["deep_eval_results"] = res_df
                                        status.update(label="Evaluation Complete!", state="complete", expanded=False)
                                    finally:
    ...
    if "deep_eval_results" in st.session_state:
        rdf = st.session_state["deep_eval_results"]
        st.markdown("### 📈 Deep Evaluation Metrics")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Avg F1", f"{rdf['f1'].mean():.2f}")
        m2.metric("Avg Precision", f"{rdf['precision'].mean():.2f}")
        m3.metric("Avg Recall", f"{rdf['recall'].mean():.2f}")
        m4.metric("Avg Confidence", f"{rdf['overall_conf'].mean():.1%}")
        
        st.dataframe(rdf[["id", "question", "f1", "overall_conf", "time"]], use_container_width=True)
        
        csv = rdf.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Download Detailed Deep Results", csv, "deep_eval_results.csv", "text/csv")
        if st.button("🗑️ Clear Evaluation Results"):
            del st.session_state["deep_eval_results"]
            st.rerun()
    ...
    if st.button("🔍 Perform Deep Analysis", use_container_width=True):
        if not f_sum or not vertex_project or not q_sum.strip(): st.error("Missing Files/Vertex Project ID/Query"); st.stop()
        ...
        with st.status("Analyzing Documents...", expanded=True) as status:
            ...
            # Initialize embedding manager for confidence metrics (Main Thread)
            _em_conf = st.session_state.get("sem_v2")
            if not _em_conf:
                try: 
                    # Try to create a new manager with the standard RAG model
                    _em_conf = EmbeddingManagerRAG("all-MiniLM-L6-v2")
                except Exception as _em_err:
                    # If that fails, it might be a caching or async issue. 
                    # The confidence scores will use a 0.5 baseline if this is None.
                    _em_conf = None
