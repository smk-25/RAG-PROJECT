...
def rouge_n_recall(pred: str, gold: str, n: int = 1) -> float:
    def ngrams(tokens, n):
        if len(tokens) < n:
            return []
        return [tuple(tokens[i:i+n]) for i in range(len(tokens)-n+1)]
    ...
    g_counts = collections.Counter(g_ngrams)
    match = 0
    for ng, cnt in g_counts.items():
        match += min(cnt, p_counts.get(ng, 0))
    return match / max(1, sum(g_counts.values()))

def rouge_1(pred: str, gold: str) -> float:
    return rouge_n_f1(pred, gold, n=1)

def rouge_1_recall(pred: str, gold: str) -> float:
    return rouge_n_recall(pred, gold, n=1)
...
def strip_provenance(text: str) -> str:
    ...
    # Remove references like "According to [source]", "Based on source", "As per document"
...
class SparseBM25Index:
    ...
    def save(self, filepath: str):
        """Persist the BM25 index and document data to disk."""
        ...
        try:
            ...
            with open(filepath, "wb") as f:
                pickle.dump(data, f)
            st.write(f"Sparse BM25 index persisted to {filepath}")
        except Exception as e:
            st.error(f"Failed to save BM25 index: {e}")

    def load(self, filepath: str) -> bool:
        """Load the BM25 index and document data from disk."""
        ...
        try:
            ...
            st.error(f"Failed to load BM25 index: {e}")
            return False
...
def compute_mean_support_score(results: List[Dict[str, Any]]) -> float:
    ...
    scores = []
    for r in results:
        s = float(r.get("similarity_score", 0.0))

        # If score looks like cross-encoder logit → normalize
        if s < 0.0 or s > 1.0:
...
st.sidebar.markdown("### Relevance Thresholds for Retrieval Metrics")
eval_token_overlap_threshold = st.sidebar.slider(
    "Token Overlap Threshold (Jaccard)",
    0.0, 1.0, 0.25, 0.05,
    help="Default 0.25. Minimum token overlap to consider a chunk relevant."
)
eval_bertscore_threshold = st.sidebar.slider(
    "BERTScore Relevance Threshold",
    -1.0, 1.0, -0.5, 0.05,
    help="Default -0.5 (rescaled). Minimum BERTScore to consider a chunk relevant."
)
eval_cosine_threshold = st.sidebar.slider(
    "Cosine Similarity Relevance Threshold",
    0.0, 1.0, 0.3, 0.05,
    help="Default 0.3. Minimum embedding similarity to consider a chunk relevant."
)
eval_key_term_threshold = st.sidebar.slider(
    "Key Term Match Threshold",
    0.0, 1.0, 0.4, 0.05,
    help="Default 0.4. Minimum fraction of key terms present in chunk."
)
# >>> NEW: Low confidence control (support threshold)
st.sidebar.markdown("### Low Confidence Control")
support_threshold = st.sidebar.number_input(
    "Support threshold (mean similarity score)",
    value=0.35,
    min_value=0.0,
    max_value=1.0,
    step=0.01,
    help="If mean similarity score of retrieved chunks is below this, answer is flagged as LOW CONFIDENCE"
)
...
if uploaded_files and len(uploaded_files) > 0:
    ...
    with st.spinner("Processing PDFs..."):
        ...
        if not all_pages:
            ...
            bm25_save_path = os.path.join("./vector_store", "bm25_index.pkl")
            ...
            if vector_store.collection.count() == 0 or reindex:
                    ...
                    try:
                        ...
                        if use_sparse_prefilter and BM25_AVAILABLE:
                            ...
                            if bm25_index.is_ready():
                                ...
                                bm25_index.save(bm25_save_path)
                            else:
                ...
                # Attempt to load existing BM25 index if requested
                if use_sparse_prefilter and BM25_AVAILABLE:
                    bm25_index = SparseBM25Index()
                    if bm25_index.load(bm25_save_path):
                        st.success(f"Loaded existing Sparse BM25 index from {bm25_save_path}")
                    else:
                        st.info("No existing Sparse BM25 index found or failed to load. BM25 prefilter will be inactive until re-indexed.")
                        bm25_index = None
                else:
                    bm25_index = None
            ...
            if q_in:
                with st.spinner("Retrieving and generating answer..."):
                    ...
                    # >>> NEW: mean support score + low confidence flag
                    mean_support_score = compute_mean_support_score(results)

                    low_confidence_flag = mean_support_score < support_threshold
                    ...
                    # >>> NEW: low confidence display
                    if low_confidence_flag:
                        st.warning(
                            f"⚠ Low confidence answer (mean support score = {mean_support_score:.3f})"
                        )
                    else:
                        st.success(
                            f"✔ Sufficient support (mean support score = {mean_support_score:.3f})"
                        )
            ...
            if eval_df is not None and st.button("Run Evaluation"):
                ...
                def compute_retrieval_stats(cand_list, gold_text: str, 
                                          token_overlap_threshold: float = 0.25,
                                          bertscore_threshold: float = -0.5,
                                          cosine_threshold: float = 0.3,
                                          key_term_threshold: float = 0.4):
                    ...
                    if sem_method == "bert_score_rescaled":
                        RELEVANCE_THRESHOLD = bertscore_threshold
                    else:
                        RELEVANCE_THRESHOLD = cosine_threshold
                    ...
                    for i, score in enumerate(sem_scores):
                        ...
                        if gold_tokens and retrieved_tokens:
                            ...
                            token_overlap = 0.0
                        has_high_token_overlap = token_overlap >= token_overlap_threshold
                        has_moderate_token_overlap = token_overlap >= (token_overlap_threshold * 0.6)  # Relative to threshold
                        ...
                        if len(gold_key_tokens) >= 2:  # Need at least 2 key tokens
                            ...
                            has_key_terms = key_token_overlap >= key_term_threshold
                            has_some_key_terms = key_token_overlap >= (key_term_threshold * 0.6)
                    ...
                    return {
                        ...
                        "overall_recall": 1 if found_rank > 0 else 0,
                        "r1": r1, "r3": r3, "r5": r5,
                        "p1": p1, "p3": p3, "p5": p5,
                ...
                for i, r in eval_df.iterrows():
                    ...
                    if do_compare_dense_sparse:
                        ...
                        stats_dense = compute_retrieval_stats(
                            cand_dense, gold,
                            token_overlap_threshold=eval_token_overlap_threshold,
                            bertscore_threshold=eval_bertscore_threshold,
                            cosine_threshold=eval_cosine_threshold,
                            key_term_threshold=eval_key_term_threshold
                        )
                        stats_sparse = compute_retrieval_stats(
                            cand_sparse, gold,
                            token_overlap_threshold=eval_token_overlap_threshold,
                            bertscore_threshold=eval_bertscore_threshold,
                            cosine_threshold=eval_cosine_threshold,
                        ...
                        stats_primary = compute_retrieval_stats(
                            cand_primary, gold,
                            token_overlap_threshold=eval_token_overlap_threshold,
                            bertscore_threshold=eval_bertscore_threshold,
                            cosine_threshold=eval_cosine_threshold,
                    ...
                    generated_raw = ""
                    generated_clean = ""
                    em = 0; f1 = 0.0; token_prec = 0.0; token_rec = 0.0
                    rouge1_score = 0.0; rouge1_rec = 0.0; rougeL_score = 0.0; rougeL_rec = 0.0
                    gen_time = 0.0
                    
                    # Compute retrieval-based metrics common to all modes
                    mean_support_score = compute_mean_support_score(cand_for_llm)
                    low_confidence_flag = mean_support_score < support_threshold
                    total_time_sec = float(rt_elapsed + gen_time)
                    ...
                    if run_llm:
                        ...
                        if ctx:
                            ...
                            token_rec = token_recall(generated_clean, gold)
                            rouge1_score = rouge_1(generated_clean, gold)
                            rouge1_rec = rouge_1_recall(generated_clean, gold)
                            rougeL_score = rouge_l(generated_clean, gold)
                            rougeL_rec = rouge_l_recall(generated_clean, gold)
                        else:
                            generated_raw = ""
                            generated_clean = ""
                            em = 0; f1 = 0.0; token_prec = 0.0; token_rec = 0.0
                    ...
                    row_dict = {
                        ...
                        "rouge_1_recall": rouge1_rec,
                        "rouge_l": rougeL_score,
                        "rouge_l_recall": rougeL_rec
                    }
                    # >>> NEW: confidence + total time columns
                    row_dict.update({
                        "mean_support_score": mean_support_score,
                        "low_confidence_flag": int(low_confidence_flag)
                    })
                    ...
                    if do_compare_dense_sparse:
                        ...
                        row_dict.update({
                            ...
                            "retrieval_recall_dense": stats_dense["overall_recall"],
                        ...
                        row_dict.update({
                            ...
                            "retrieval_recall_sparse": stats_sparse["overall_recall"],
                        ...
                        row_dict.update({
                            ...
                            "retrieval_recall": stats_primary["overall_recall"],
                ...
                if do_compare_dense_sparse:
                    ...
                    st.write(f"- Retrieval Recall: {res_df['retrieval_recall_dense'].mean():.3f}")
                    ...
                    st.write(f"- Retrieval Recall: {res_df['retrieval_recall_sparse'].mean():.3f}")
                    ...
                    st.write(f"- Retrieval Recall: {res_df['retrieval_recall'].mean():.3f}")
                ...
                if run_llm:
                    ...
                    st.write(f"- Token Recall: {res_df['token_recall'].dropna().mean():.3f}")
                    st.write(f"- ROUGE-1 F1: {res_df['rouge_1'].dropna().mean():.3f}")
                    st.write(f"- ROUGE-1 Recall: {res_df['rouge_1_recall'].dropna().mean():.3f}")
                    st.write(f"- ROUGE-L F1: {res_df['rouge_l'].dropna().mean():.3f}")
                    st.write(f"- ROUGE-L Recall: {res_df['rouge_l_recall'].dropna().mean():.3f}")
                ...
                st.subheader("Sample per-question results (first 30 rows)")
